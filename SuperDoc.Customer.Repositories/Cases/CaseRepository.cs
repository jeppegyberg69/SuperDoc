...
namespace SuperDoc.Customer.Repositories.Cases
{
    public class CaseRepository : ICaseRepository
    {
        ...
        public async Task<Case?> GetCaseByIdWithCaseManagersAndResponsibleUserAsync(Guid caseId)
        {
            return await superDocContext.Cases.Include(x => x.CaseManagers).Include(x => x.ResponsibleUser).FirstOrDefaultAsync(x => x.CaseId == caseId);
        }
        ...
        public async Task UpdateCase(Case docCase)
        {
            ...
            // This field is not allowed to be updated because it is generated by the database
            superDocContext.Entry(docCase).Property(x => x.CaseNumber).IsModified = false;
            await superDocContext.SaveChangesAsync();
        }
